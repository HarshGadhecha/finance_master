    ...
    <SafeAreaView style={{ backgroundColor: 'white', flex: 1 }}>
      ...
      <ScrollView style={{ padding: 20 }} showsVerticalScrollIndicator={false}>
        <TouchableWithoutFeedback onPress={() => { Keyboard.dismiss() }}>
          <View style={{ flex: 1 }}>
            <BackButton />
            <Text style={{ fontSize: 20, marginVertical: 20, fontWeight: 'bold' }}>Present Value Calculation : </Text>

            <InputPrincipal text={'FUTURE VALUE'} value={futureValue} onChangeText={(text) => { setPresentValue(); setFutureValue(text) }} />

            <View style={{ flexDirection: 'row', marginVertical: 10, alignItems: 'center' }}>
              <View style={{ marginRight: 10 }}>
                <InputInterest text={'INTEREST RATE (%)'} value={interestRate} onChangeText={(text) => { setPresentValue(); setInterestRate(text) }} />
              </View>
              <View>
                <InputInterest text={'YEARS :'} value={years} onChangeText={(text) => { setPresentValue(); setYears(text) }} />
              </View>
            </View>
            ...
            <View>
              {presentValue > 0 && (
                <Animated.View style={{ borderRadius: 25, borderWidth: 1, paddingVertical: 20 }} entering={entering} exiting={exiting}>
                  ...
                  <CalculationText />
                    <PieChart principal={futureValue} interest={-(futureValue-presentValue)} data1={'Present Value'} data2={'Future Value'} />
